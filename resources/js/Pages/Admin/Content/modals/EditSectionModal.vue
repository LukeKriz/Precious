...
const props = defineProps({
  ...
  initialBg: { type: String, default: "" },

  initialBgImage: { type: String, default: "" },
  initialOverlayColor: { type: String, default: "" },
  initialOverlayOpacity: { type: Number, default: 0 },
  initialPaddingTop: { type: Number, default: 0 },
  initialPaddingBottom: { type: Number, default: 0 },

  // ✅ NOVÉ
  initialVerticalAlign: { type: String, default: "center" }, // top|center|bottom
...
watch(
  ...
  (v) => {
    ...
    height.value = Number(props.initialHeight ?? 300);

    bg.value = String(props.initialBg ?? "").trim();
    bgImage.value = String(props.initialBgImage ?? "").trim();

    overlayColor.value = String(props.initialOverlayColor ?? "").trim();
    overlayOpacity.value = Math.max(
      0,
      Math.min(100, Number(props.initialOverlayOpacity ?? 0))
    );

    paddingTop.value = Math.max(0, Number(props.initialPaddingTop ?? 0));
    paddingBottom.value = Math.max(0, Number(props.initialPaddingBottom ?? 0));

    // ✅ init va
    va.value = normalizeVA(props.initialVerticalAlign ?? "center");
  }
);
  ...
  <div v-if="open" class="fixed inset-0 z-[9999] flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-3xl p-6">
      <div class="flex items-center justify-between mb-4">
        <div class="text-lg font-bold">Upravit sekci</div>
        ...
        <div class="relative w-full h-40">
          ...
          <div
            ...
            :style="{
              ...
              opacity: (overlayOpacity / 100).toString(),
            }"
          />
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <div class="flex items-center justify-between text-sm mb-1">
            <div class="font-medium">Opacity</div>
            <div class="text-gray-500">{{ overlayOpacity }}%</div>
          </div>
